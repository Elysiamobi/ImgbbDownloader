...
def rename_duplicates():
    ...
    with _lock:
        with open(task_status_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        # Track filenames per subfolder to avoid cross-folder dedup
        folder_filename_count = {}
        updated = False
        for url, info in data.items():
            subfolder = info.get("subfolder", "")
            if subfolder not in folder_filename_count:
                folder_filename_count[subfolder] = {}
            filename_count = folder_filename_count[subfolder]
...
def get_failed_map(log_func=print):
    try:
        ...
        with open(task_status_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        return {
            url: (info["filename"], info.get("subfolder", ""))
            for url, info in data.items()
