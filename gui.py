...
class ImgbbDownloaderApp(QWidget):
    def __init__(self):
        ...
        app_state.max_workers = self.config.get("max_workers", 6)
    ...
    def init_ui(self):
        ...
        thread_layout.addWidget(QLabel("⚡ 下载线程数:"))

        self.thread_spinbox = QSpinBox()
        self.thread_spinbox.setRange(1, 64)
        self.thread_spinbox.setValue(app_state.max_workers)
    ...
    def update_progress_signal(self, file_index, total_files, file_name, status, subfolder=""):
        if status != "已完成":
        ...
        if status in ["已完成", "失败"]:
            ...
            if status == "已完成":
                target_dir = os.path.join(app_state.download_dir, subfolder) if subfolder else app_state.download_dir
    ...
    def start_new_task(self):
        ...
        def worker():
            try:
                ...
                for file_info in url_map.values():
    ...
    def on_thread_count_changed(self, value):
        app_state.max_workers = value
        self.config["max_workers"] = value
        write_config(self.config)
        self.log(f"⚡ 下载线程数已设置为: {value}")
