...
def download_file(download_url, file_name, log_func=print,
    ...
    if head_ok is True:
        ...
        if progress_callback:
            progress_callback(file_index, total_files, file_name, "已完成", subfolder)
        return None
    ...
    if progress_callback:
        progress_callback(file_index, total_files, file_name, "下载中", subfolder)
    ...
    if progress_callback:
        progress_callback(file_index, total_files, file_name, "失败", subfolder)
...
def download_files_concurrently(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for idx, (url, file_info) in enumerate(urls, 1):
            if shutdown_event.is_set():
                break

            if isinstance(file_info, tuple):
                filename, subfolder = file_info
            else:
                filename, subfolder = file_info, ""
